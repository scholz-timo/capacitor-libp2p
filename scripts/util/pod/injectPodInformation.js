const findIndexFromPosition = (arr, index, callback) => {
  return arr.findIndex((value, searchIndex, arr) =>
    searchIndex <= index ? false : callback(value, searchIndex, arr),
  );
};
...
const findAndInjectIntoApp = podLines => {
  const appTargetIndex = podLines.findIndex(line =>
    line.trim().startsWith('target'),
  );
  ...
  const appTargetEnd = findIndexFromPosition(
    podLines,
    appTargetIndex,
    value => value.trim() === 'end',
  );
  ...
  const previousInstallation = findIndexFromPosition(
    podLines,
    appTargetIndex,
    (value, index) => (index <= appTargetEnd ? value.trim() === fnName : false),
  );
...
const injectPodFn = (__podLines, podTargetLines) => {
  ...
  let installPosition = podLines.findIndex(
    line => line.trim() === injectPodFnLine,
  );

  if (installPosition === -1) {
    const installLine = podLines.findIndex(line =>
  ...
  const deleteLineIndex = findIndexFromPosition(
    podLines,
    installPosition,
    line => line.trim() === 'end',
  );
  ...
  podLines.splice(
    installPosition + 1,
    deleteLineIndex - 1 - installPosition,
    ...podTargetLines,
  );
  return podLines;
};
...
const injectPostinstallScript = podLines => {
  const hasPostinstallInstalled = podLines.findIndex(line =>
    line.trim().startsWith('$postinstall.each'),
  );
  ...
  const installPosition = podLines.findIndex(line =>
    line.trim().startsWith('post_install'),
  );
  if (installPosition === -1) {
