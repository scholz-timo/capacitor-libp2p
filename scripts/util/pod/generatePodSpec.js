...
const generatePodSpec = (packageContent, podspecSources = {}) => {
    ...
    if (currentPackageJSON?.['podspec']?.['overrides']?.[packageContent.packageContent['identity']] !== undefined) {
        ...
        const results = currentPackageJSON?.['podspec']?.['overrides']?.[packageContent.packageContent['identity']];
    ...
    const possibleOverrideDirectory = Path.join(__dirname, '..', '..', '..', 'podspecs', packageContent.packageContent['identity']);
    ...
    return [{ isFile: true, dependency: packageContent.packageContent['identity'], podspecPath }];
}
