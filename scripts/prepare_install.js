...
const {
  generatePodSpec,
  generatePodSpecId,
} = require('./util/pod/generatePodSpec');
...
const resolvedPackagesFile = Path.join(
  __dirname,
  '..',
  'ios',
  'Plugin.xcworkspace',
...
resolvePackages(resolvedPackagesFile).then(async packages => {
  ...
  await Promise.all(
    packages.map(async packageContent => {
      const {
        location,
        identity,
        state: { revision },
      ...
      if (
        !Object.keys(myPackageJSON?.['podspec']?.['overrides'] ?? {}).includes(
          identity,
        )
      ) {
        await checkout(targetDir, location, revision);
        innerDependencies = await resolvePackages(
          Path.join(targetDir, 'Package.resolved'),
          packages,
        );
      }
      ...
      storedPackages[generatePodSpecId(packageContent)] = {
        packageContent,
        dependencies: innerDependencies.filter(({ identity }) =>
          availableIdentities.includes(identity),
        ),
        targetDir,
      };
    }),
  ...
  do {
    const packagesWithoutDependencies = storedPackageEntries.filter(
      ([, value]) => value.dependencies.length === 0,
    );
    const packagesWithoutDependenciesNames = packagesWithoutDependencies.map(
      ([name]) => name,
    );
    ...
    const newStoredPackageEntries = storedPackageEntries
      .filter(([name]) => {
      ...
      .map(([name, packages]) => {
        return [
          ...
          {
            ...packages,
            dependencies: packages.dependencies.filter(
              dependency =>
                !packagesWithoutDependenciesNames.includes(
                  generatePodSpecId(dependency),
                ),
            ),
          },
        ];
    ...
    for (const packageName of packagesWithoutDependenciesNames) {
      ...
      const specs = (podspecSources[
        generatePodSpecId(packageContent.packageContent)
      ] = generatePodSpec(packageContent, podspecSources));

      specs
        .filter(spec => spec.isFile)
        .forEach(spec => {
          podTargetLines.push(
            `pod "${spec.dependency}", :path => "${Path.relative(
              currentIOSPath,
              spec.podspecPath,
            )}"`,
          );
