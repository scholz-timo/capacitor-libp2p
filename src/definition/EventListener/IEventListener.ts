...
> = Event extends keyof TypeRecord
  ? GetTypesForEntry<Event, TypeRecord> extends any[]
    ? Concat<DefaultEventCallbackParams, Exclude<TypeRecord[Event], undefined>>
    : DefaultEventCallbackParams
  : DefaultEventCallbackParams;

export type AllToUndefined<Tuple extends any[]> = CompareLength<
  Tuple,
  []
> extends 'equal'
  ? [Tuple[0] | undefined, ...AllToUndefined<Drop<Tuple, 1>>]
  : [];
type NormalizeLength<
  Tuple extends any[],
  ourLongestTuple extends any[],
> = CompareLength<Tuple, ourLongestTuple> extends 'equal'
  ? Tuple
  : Append<Tuple, undefined>;

type __NormalizeEventParameters<
  Tuple extends any[][],
  ourLongestTuple extends any[],
> = Tuple['length'] extends 1
  ? NormalizeLength<Tuple[0], ourLongestTuple>
  :
      | __NormalizeEventParameters<Drop<Tuple, 1>, ourLongestTuple>
      | NormalizeLength<Tuple[0], ourLongestTuple>;

export type NormalizeEventParameters<Tuple extends any[]> = EnsureArrayType<
  Tuple['length'] extends 0
    ? Tuple
    : UnionToArray<Tuple> extends [any[], ...any[]]
    ? __NormalizeEventParameters<
        Exclude<UnionToArray<Tuple>, LongestTuple<UnionToArray<Tuple>>>,
        LongestTuple<UnionToArray<Tuple>>
      >
    : Tuple
>;

type EnsureArrayType<PotentialArrayType> = PotentialArrayType extends any[]
  ? PotentialArrayType
  : [];
...
> {
  ...
  off<Event extends keyof EventList>(
    ...
    ...args: AllToUndefined<SpecialOnArgs>
  ): this;
}
